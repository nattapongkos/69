...
window.onload = async () => {
  ...
  if (status) status.textContent = "Loading Interface...";
  ...
  if (status) status.textContent = "Authenticating Session...";
  ...
  if (status) status.textContent = "Applying System Settings...";
  await applySystemToggles();

  if (status) status.textContent = "Syncing User Profile...";
  await syncProfileWithGachaData();
  // ---------------------------------------------------------

  if (bar) bar.style.width = "100%";
  if (status) status.textContent = "Portal Ready!";

  // หน่วงเวลาให้เห็น 100% แป๊บนึงแล้วค่อยปิดหน้า Loading
  setTimeout(() => {
    hideLoading();
  }, 800);
};
...
// 2. ฟังก์ชันสลับหน้า (หัวใจสำคัญ)
function switchTab(tabName) {
  // รายชื่อ ID ของ Panel ทั้งหมดที่คุณมีในหน้า index.html
  const allPanels = [
    "home",
    "submit",
    "halloffame",
  ...
  // 🔄 ขั้นตอนที่ 1: สั่งซ่อน (hidden) ทุก Panel ก่อน
  allPanels.forEach((id) => {
    const el = document.getElementById(`panel-${id}`);
    if (el) el.classList.add("hidden");
  });

  // ✨ ขั้นตอนที่ 2: สั่งแสดงเฉพาะหน้าที่เรากดเลือก
  const targetPanel = document.getElementById(`panel-${tabName}`);
  if (targetPanel) {
    targetPanel.classList.remove("hidden");
    // เลื่อนหน้าจอกลับไปบนสุดทุกครั้งที่เปลี่ยนหน้า
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  // (ส่วนนี้เป็น Option เสริม) ปรับปุ่มกดย้อนกลับให้แสดงเมื่อไม่อยู่หน้า Home
  const backBtn = document.getElementById("btn-back-home");
  if (backBtn) {
    if (tabName === "home") backBtn.classList.add("hidden");
    else backBtn.classList.remove("hidden");
  }

  // อัปเดตไอคอน Lucide (ถ้ามีการใช้)
  if (typeof lucide !== "undefined") lucide.createIcons();
}
...
function switchSubTab(sub) {
  document
    .getElementById("view-form")
    .classList.toggle("hidden", sub !== "form");
  document
    .getElementById("view-hist")
    .classList.toggle("hidden", sub !== "hist");
  const btnForm = document.getElementById("subtab-form");
  const btnHist = document.getElementById("subtab-hist");
  if (sub === "form") {
    btnForm.className =
      "px-5 py-2.5 bg-white text-indigo-600 shadow-sm rounded-lg text-sm font-bold transition-all";
    btnHist.className =
      "px-5 py-2.5 text-slate-500 hover:bg-white/80 rounded-lg text-sm font-bold transition-all";
  } else {
    btnForm.className =
      "px-5 py-2.5 text-slate-500 hover:bg-white/80 rounded-lg text-sm font-bold transition-all";
    btnHist.className =
...
function checkDuplicateSubmission() {
  ...
  if (existingSubmissions.length > 0) {
    btn.innerHTML =
      '<i data-lucide="refresh-cw" style="width:20px;height:20px"></i> ส่งงานซ้ำ (ทับของเดิม)';
    btn.className =
      "w-full py-4 rounded-2xl bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold text-base hover:from-orange-600 hover:to-red-700 shadow-lg flex items-center justify-center gap-2 tracking-wide transition-all";
    if (alertBox) {
    ...
    btn.innerHTML =
      '<i data-lucide="send" style="width:20px;height:20px"></i> ยืนยันการส่งงาน';
    btn.className =
      "w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold text-base hover:from-indigo-700 hover:to-purple-700 shadow-lg flex items-center justify-center gap-2 tracking-wide transition-all";
...
function renderFilePreview() {
  ...
  preview.innerHTML = selectedFilesArray
    .map(
      (f, i) =>
        `<div class="flex items-center justify-between bg-white px-4 py-3 rounded-2xl border border-slate-200 shadow-sm mb-2"><div class="flex items-center gap-3 overflow-hidden"><i data-lucide="file-check-2" class="w-5 h-5 text-indigo-500 shrink-0"></i><p class="text-sm font-bold text-slate-700 ">${f.name} <span class="text-xs text-slate-400">(${(f.size / 1024).toFixed(0)}KB)</span></p></div><button type="button" onclick="event.stopPropagation(); removeFileIndex(${i})" class="text-slate-400 hover:text-red-500 p-2 shrink-0 transition-colors"><i data-lucide="trash-2" class="w-4 h-4"></i></button></div>`,
    )
    .join("");
  lucide.createIcons();
}
...
async function executeSubmission() {
  ...
  if (pCont) {
    ...
    pBar.className =
      "bg-gradient-to-r from-blue-500 to-indigo-600 h-full rounded-full transition-all duration-300";
  }
  ...
  try {
    ...
    if (existingSubmissions.length > 0) {
        for (let oldSub of existingSubmissions) {
          // 🟢 1. สั่งลบไฟล์ใน Google Drive ก่อน (ถ้ามี fileId)
          if (oldSub.fileId) {
             try {
               await fetch(UPLOAD_GAS_URL + "?action=deleteFile&fileId=" + oldSub.fileId);
             } catch (err) {
               console.warn("ไม่สามารถลบไฟล์เก่าใน Drive ได้ แต่ะลบข้อมูลในระบบต่อ");
             }
          }
    ...
    if (selectedFilesArray.length > 0) {
      if (!UPLOAD_GAS_URL || UPLOAD_GAS_URL.includes("วาง_URL"))
        throw new Error("ยังไม่ได้ตั้งค่า Web App URL");
      for (let i = 0; i < selectedFilesArray.length; i++) {
        updateProgress(
          `กำลังส่งไฟล์เข้า Drive ${i + 1}/${selectedFilesArray.length}...`,
          20 + (i / selectedFilesArray.length) * 60,
        );
        const filePayload = {
          action: "upload_file",
          data: {
            name: selectedFilesArray[i].name,
            type: selectedFilesArray[i].type,
            base64: selectedFilesArray[i].base64,
          },
        };
        const fileJson = await uploadFileToGAS(UPLOAD_GAS_URL, filePayload);
        uploadedFilesData.push(fileJson.file);
      }
    }
...
function renderHistory() {
  ...
  if (!sorted.length) {
    el.innerHTML = `<div class="text-center py-16 text-slate-400 font-bold"><p>ไม่พบข้อมูลประวัติการส่งงาน</p></div>`;
    return;
  }
      ...
      let fileLinks =
        urls.length > 0
          ? urls
              .map(
                (u, i) =>
                  `<a href="${u}" target="_blank" class="flex items-center gap-1.5 text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-xl text-xs font-bold transition-colors border border-blue-100"><i data-lucide="file-check-2" class="w-3.5 h-3.5"></i> ${names[i] || "ไฟล์แนบ"}</a>`,
              )
              .join("")
          : '<span class="text-slate-400 text-xs font-bold bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">ไม่มีไฟล์แนบ</span>';
      const st = getStatus(item);

      return `<div class="bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow"><div class="flex items-start gap-4"><div class="p-3 bg-indigo-50 text-indigo-500 rounded-2xl shrink-0"><i data-lucide="folder-check" class="w-6 h-6"></i></div><div class="flex-1 min-w-0"><div class="flex items-center gap-2 mb-2"><h3 class="text-base font-bold text-slate-800">${item.title}</h3><span class="text-[10px] font-bold px-2.5 py-1 rounded-lg border ${st.cls} ml-2">${st.label}</span></div><div class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4 flex flex-wrap gap-3"><span>รหัส: <span class="text-indigo-500">${item.student_id}</span></span> <span>วิชา: ${item.subject}</span> <span>ส่ง: ${formatDate(item.submitted_at)}</span></div><div class="flex flex-wrap gap-2">${fileLinks}</div></div></div></div>`;
    })
    .join("");
  lucide.createIcons();
}
...
function executeLogout() {
  localStorage.removeItem("student_session"); // ลบประวัติล็อกอิน
...
// ========================================================
// 🌟 ระบบทำเนียบนักล่าสมบัติ (เวอร์ชันอัปเกรด: กรองนักเรียนที่ถูกลบ + Cleanup ขยะอัตโนมัติ)
// ========================================================
async function loadTreasureShowcase() {
  ...
  try {
    // 1. ดึงข้อมูลนักเรียนที่ยังมีตัวตนอยู่จริงในระบบ (Active Students)
    const studentsSnap = await db.collection("students").get();
    const activeStudentIds = new Set();
    studentsSnap.docs.forEach(doc => activeStudentIds.add(String(doc.data().student_id)));

    // 2. ดึงข้อมูลภารกิจสมบัติจาก Firebase
    const qSnap = await db.collection("treasure_quests").get();
    const questsData = {};
    qSnap.docs.forEach((d) => (questsData[d.id] = { id: d.id, ...d.data() }));

    const res = await fetch(TREASURE_GAS_URL, {
      method: "POST",
      body: JSON.stringify({ action: "get_showcase" }),
      headers: { 
        "Content-Type": "text/plain;charset=utf-8" // ต้องใช้ text/plain ถูกแล้วครับ
      },
      redirect: "follow" // 🌟 เพิ่มบรรทัดนี้ เพื่อให้มันตามการ redirect 302 ของ Google
    });
    const json = await res.json();
    ...
    // 4. จัดกลุ่มและกรองข้อมูล (เฉพาะนักเรียนที่ยังมีตัวตน)
    let studentTreasuresMap = {};
    let orphanedIds = new Set(); // เก็บ ID ที่ตกค้างเพื่อไปสั่งลบใน GAS

    json.data.forEach((data) => {
      const stuId = String(data.student_id);
      
      if (activeStudentIds.has(stuId)) {
        // ✅ นักเรียนยังมีตัวตน: เก็บข้อมูลไว้แสดงผล
        if (!studentTreasuresMap[stuId]) studentTreasuresMap[stuId] = [];
        if (questsData[data.quest_id]) studentTreasuresMap[stuId].push(questsData[data.quest_id]);
      } else {
        // ❌ นักเรียนถูกลบไปแล้ว: เตรียมส่งไปลบใน Sheets
        orphanedIds.add(stuId);
      }
    });

    // 🧹 5. Auto-Cleanup: ถ้าเจอข้อมูลค้างใน Sheets แต่ไม่มีตัวตนในระบบ ให้ส่งคำสั่งไปลบที่ GAS
    if (orphanedIds.size > 0) {
      orphanedIds.forEach(id => {
        fetch(TREASURE_GAS_URL, {
          method: "POST",
          body: JSON.stringify({ action: "delete_student_records", studentId: id }),
          headers: { "Content-Type": "text/plain;charset=utf-8" },
        }).catch(err => console.warn("Cleanup GAS failed for ID:", id));
      });
    }

    // 6. แปลงข้อมูลเตรียมโชว์หน้าจอ (เหมือนเดิมแต่สะอาดขึ้น)
    allTreasureHunters = [];
    for (let stuId in studentTreasuresMap) {
      ...
      if (!studentInfo) continue; // กันเหนียวไว้อีกชั้น
    ...
    console.error("Treasure Showcase Error:", error);
    showcaseEl.innerHTML = '<div class="text-rose-500 text-sm font-bold text-center py-8">เกิดข้อผิดพลาดในการโหลดข้อมูล</div>';
  }
}
...
async function handleProfileUpload(event) {
  ...
  // 🚨 ตรวจสอบว่า URL เป็นตัวล่าสุดที่ได้จากการ New Deployment หรือไม่
  ...
  try {
    ...
    // 3. ยิงไปที่ GAS Web App
    const response = await fetch(GAS_UPLOAD_URL, {
      method: "POST",
      body: JSON.stringify({
        action: "upload_profile", // 🚨 เพิ่มตัวนี้!
        data: {
    ...
    console.log("GAS Response:", rawResponse); // ดูค่าใน Console เพื่อตรวจสอบ

    let result;
    try {
      result = JSON.parse(rawResponse);
    } catch (e) {
      // ถ้ายังเป็น OK จะเด้งเตือนตรงนี้
      throw new Error(
        `Server ตอบกลับผิดพลาด: ${rawResponse} (กรุณาทำ New Deployment ที่ GAS)`,
      );
    }

    if (result.status === "success") {
      const driveUrl = `https://drive.google.com/thumbnail?id=${result.fileId}&sz=w500`;
      ...
      if (!snapshot.empty) {
        await db.collection("students").doc(snapshot.docs[0].id).update({
          profile_url: driveUrl,
        });
      }

      // อัปเดตหน้าจอ
      const avatarImg = document.querySelector("#main-profile-avatar img");
      if (avatarImg) avatarImg.src = driveUrl;
      else
        document.getElementById("main-profile-avatar").innerHTML =
          `<img src="${driveUrl}" class="w-full h-full rounded-full object-cover">`;
